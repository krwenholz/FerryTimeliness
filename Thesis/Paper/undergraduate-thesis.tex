...
One concern during this process is that the ferry system itself is considered on 
...
% TODO: Add a little discussion about our accuracies near the end of this in 
% comparison to the ``stupid'' model that has 87% accuracy (lead in to p/r disc.)
...
Another possible explanation is the robustness of this ferry system. Recall we that
...
but so far we have only used accuracy as a measure of effectiveness. We now 
...
Here we are considering positive to mean a trip is flagged as 
late and negative as not-late. The diagonal of the matrix tells us the times our
classifier is correct, and non-zero values off the diagonal indicate incorrect
classifications. Thus, the precision is calculated as 
...
likely to be late than not-late. High recall means our model is catching most of
...
such a score. We will look at scores for the weather/no-weather models of 
departure tardiness from \textbf{Section~\ref{sec:gen_results}}, our best and most
interesting model.
